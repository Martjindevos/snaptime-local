...
export default function ClassSelect({ classes, schoolName, students: initialStudents, onSelect, onBack }: Props) {
  ...
  // Get classes from allStudents (includes newly created classes)
  const filteredClasses = Object.keys(allStudents).filter(c => c.toLowerCase() !== 'stamgroep').sort()
  ...
  const handleSelectStudent = (student: Student & { className: string }) => {
    onSelect(student.className, allStudents, student.id)
  }
              ...
              {!showNewClassInput ? (
                <>
                  <select
                    ...
                    style={{ flex: 1, padding: '8px', border: '1px solid #d1d5db', borderRadius: '4px', fontSize: '12px', boxSizing: 'border-box' }}
                  >
                  ...
                  <button
                    onClick={() => { setShowNewClassInput(true); setNewStudent({ ...newStudent, className: '' }) }}
                    style={{
                      padding: '8px 12px',
                      background: '#10b981',
                      color: 'white',
                ...
                <>
                  <input
                    type="text"
                    placeholder="New class/group name (e.g., Teachers, Concierge)"
                    value={newStudent.className}
                    onChange={(e) => setNewStudent({ ...newStudent, className: e.target.value })}
                    style={{ flex: 1, padding: '8px', border: '1px solid #d1d5db', borderRadius: '4px', fontSize: '12px', boxSizing: 'border-box' }}
                  />
